...
double? _tryParseDouble(Uint8List bytes, int start, int end) {
  try {
    ...
    if (reader.peek() != JsonTokenType.number) return null;
...
void testOversizedExponents() {
  ...
  for (final source in cases) {
    ...
    final actual = _parseDouble(bytes, 0, bytes.length);
...
void testZeroAndNegativeZero() {
  ...
  final zeroParsed = _parseDouble(zeroBytes, 0, zeroBytes.length);
  Expect.equals(0.0, zeroParsed);
  Expect.isFalse(zeroParsed.isNegative);
  Expect.equals((0, 0), zeroParsed.toBits());

  // Negative zero
  final negZeroBytes = _utf8Bytes("-0.0");
  final negZeroParsed = _parseDouble(negZeroBytes, 0, negZeroBytes.length);
  Expect.equals(0.0, negZeroParsed);
  Expect.isTrue(negZeroParsed.isNegative);
  Expect.equals((0x80000000, 0), negZeroParsed.toBits());

  // Negative integer zero
  final negIntZeroBytes = _utf8Bytes("-0");
  final negIntZeroParsed = _parseDouble(
    negIntZeroBytes,
    0,
  ...
  Expect.equals((0x80000000, 0), negIntZeroParsed.toBits());

  // Negative zero with exponent
  final negZeroExpBytes = _utf8Bytes("-0e5");
  final negZeroExpParsed = _parseDouble(
    negZeroExpBytes,
    0,
...
void testBoundaryAndTieBreaking() {
  ...
  for (final s in boundaryCases) {
    ...
    Expect.equals(
      expected.toBits(),
      actual.toBits(),
      "Bit mismatch for '$s': expected=$expected (${expected.toHex()}), got=$actual (${actual.toHex()})",
    );
  }
}
...
void testNegativeRfc8259Syntax() {
  ...
  for (final s in invalidCases) {
    ...
    Expect.throwsFormatException(
      () => _parseDouble(bytes, 0, bytes.length),
      "Should reject invalid JSON number syntax: '$s'",
    );
    Expect.equals(
      null,
      _tryParseDouble(bytes, 0, bytes.length),
...
void testFuzzDifferential100k() {
  ...
  for (int i = 0; i < 100000; i++) {
    ...
    Expect.equals(
      expected.toBits(),
      actual.toBits(),
      "Fuzz mismatch on iteration $i for '$s': expected=$expected (${expected.toHex()}), got=$actual (${actual.toHex()})",
    );
    passCount++;
  }
