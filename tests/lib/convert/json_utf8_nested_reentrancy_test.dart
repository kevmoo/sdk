...
void testDeepToJsonRecursion() {
  // Build a 100-level deep toJson() recursion chain
...
void _isolateWorker(SendPort replyPort) {
  try {
    for (var iter = 0; iter < 50; iter++) {
      ...
      // Verify span parser functions concurrently
      final dVal = JsonUtf8Decoder.parseDouble(
      ...
      final outBuf = Uint8List(64);
      final sLen = JsonUtf8Encoder.writeStringToBuffer(
        'concurrent_str',
        outBuf,
        0,
      );
      Expect.equals(16, sLen);
      Expect.equals('"concurrent_str"', utf8.decode(outBuf.sublist(0, sLen)));
    }
    replyPort.send('OK');
  } catch (e, st) {
