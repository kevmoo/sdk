...
class BazelBrowserTestE2E(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dart_sdk_bin = os.path.join(self.workspace_dir, 'tools', 'sdks',
                                         'dart-sdk', 'bin', 'dart')
        self.generator_script = os.path.join(self.workspace_dir, 'tools',
                                             'bazel', 'dart',
                                             'generate_test_targets.dart')
        self.test_py = os.path.join(self.workspace_dir, 'tools', 'test.py')
        self.bazel_output_base = os.path.join(self.workspace_dir, 'out',
                                              'bazel_e2e_ob')
    ...
    def test_r1_dynamic_vm_target_selection(self):
        ...
        res_default = subprocess.run([
            'bazel', f'--output_base={self.bazel_output_base}', 'build',
            '//sdk:create_sdk'
        ],
        ...
        res_info = subprocess.run([
            'bazel', f'--output_base={self.bazel_output_base}', 'info',
            'bazel-bin'
        ],
                                  cwd=self.workspace_dir,
                                  stdout=subprocess.PIPE,
        ...
        self.assertEqual(res_info.returncode, 0,
                         f"Failed to get bazel-bin path: {res_info.stderr}")
        bazel_bin_dir = res_info.stdout.strip()

        dart_path = os.path.join(bazel_bin_dir, 'sdk', 'dart-sdk', 'bin',
                                 'dart')
        dartaot_path = os.path.join(bazel_bin_dir, 'sdk', 'dart-sdk', 'bin',
                                    'dartaotruntime')
        self.assertTrue(os.path.exists(dart_path),
                        f"Dart VM binary missing at {dart_path}")
        self.assertTrue(os.path.exists(dartaot_path),
                        f"AOT runtime binary missing at {dartaot_path}")

        # 2. Build SDK in product mode
        print("Building product SDK...")
        res_product = subprocess.run([
            'bazel', f'--output_base={self.bazel_output_base}', 'build',
            '--//build/config:dart_product=true', '//sdk:create_sdk'
        ],
                                     cwd=self.workspace_dir,
                                     stdout=subprocess.PIPE,
                                     stderr=subprocess.PIPE,
                                     text=True)
        self.assertEqual(res_product.returncode, 0,
                         f"Product build failed:\n{res_product.stderr}")
        self.assertTrue(os.path.exists(dart_path),
                        f"Dart VM binary missing at {dart_path}")
        self.assertTrue(os.path.exists(dartaot_path),
                        f"AOT runtime binary missing at {dartaot_path}")
    ...
    def test_r2_target_generation(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            print(
                f"Running generate_test_targets.dart with output-dir={tmpdir}..."
            )
            res = subprocess.run([
                self.dart_sdk_bin, self.generator_script,
                f'--workspace-dir={self.workspace_dir}',
                f'--output-dir={tmpdir}', '--suite=corelib'
            ],
                                 stdout=subprocess.PIPE,
                                 stderr=subprocess.PIPE,
                                 text=True)
            ...
            self.assertTrue(
                os.path.exists(corelib_build_path),
                f"BUILD.bazel not generated at {corelib_build_path}")
            ...
            print(
                "Successfully verified generated target configs contain browser configurations."
            )
...
''')
            ...
            configs = [
                ('dart2wasm-chrome',
                 '@dart_tests//corelib:list_test_wasm_chrome_release'),
                ('dart2wasm-firefox',
                 '@dart_tests//corelib:list_test_wasm_firefox_release'),
                ('dart2js-chrome',
                 '@dart_tests//corelib:list_test_dart2js_chrome_release'),
                ('dart2js-firefox',
                 '@dart_tests//corelib:list_test_dart2js_firefox_release'),
            ]

            for config_name, expected_target in configs:
                print(f"Testing wrapper routing for config '{config_name}'...")
                res = subprocess.run([
                    'python3', self.test_py, '--bazel', '-n', config_name,
                    'corelib/list_test'
                ],
                                     env=env,
                                     stdout=subprocess.PIPE,
                ...
                self.assertEqual(
                    res.returncode, 0,
                    f"Wrapper failed for {config_name}:\nStdout: {res.stdout}\nStderr: {res.stderr}"
                )
