...
def main():
    ...
    repo_dir = os.path.join(output_base, "external",
                            "+dart_tests_extension+dart_tests")
    if not os.path.exists(repo_dir):
        repo_dir = os.path.join(output_base, "external", "dart_tests")

    if not os.path.exists(repo_dir):
        print(
            f"Error: Repository directory under output_base '{output_base}' does not exist. Run a bazel test first."
        )
        return 1
    ...
    for bf in build_files:
        ...
        for m in matches:
            ...
            expected_glob_re = re.compile(
                r'data\s*=\s*glob\(\s*\[\s*"gen_tests/' +
                re.escape(config_name) + r'/\*\*/\*\.dart"\s*,\s*"gen_tests/' +
                re.escape(config_name) +
                r'/\*\*/\*\.html"\s*\]\s*,\s*allow_empty\s*=\s*True\s*\)')
    ...
    if targets_missing_glob:
        print(
            f"Error: The following targets are missing the correct HTML glob pattern:"
        )
        ...
        print(
            "Success: All sh_test targets have the correct .dart and .html globbing patterns."
        )
    ...
    if improperly_routed:
        ...
        print(
            "Success: All .html files are correctly routed under gen_tests/ subdirectories."
        )
    ...
    if missing_bigint_paths:
        print(
            "Error: The following expected bigint_js_test HTML files are missing:"
        )
        ...
        print(
            "Success: Checked skipped bigint_js_test HTML wrappers, all are correctly routed."
        )
