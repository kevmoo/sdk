...
_Last updated: 2026-05-28 (session 12) — rules_dart Step 1 + Step 4 tools (dtd, dds, frontend_server) landed._
...
| 2a | `utils/` — Dart-builds-Dart | 🟡 ~15% | `rules_dart` Steps 0–2 done + Step 4 in progress: `dart_kernel_snapshot`+`dart_aot_snapshot`+`dart_compile_platform` macros (`//tools/bazel/dart`). Step 0 → `kernel_worker_aot_product`; Step 1 → `//runtime/vm:vm_platform` builds `vm_platform.dill` in-Bazel (byte-identical to GN); Step 2 → `bootstrap_gen_kernel.dill` in-Bazel; Step 4 → **dtd, dds, frontend_server ported** (AOT snapshots all run). **dartdev + dart_runtime_service_vm deferred** on out-of-band package_config/dep staleness (not rule gaps). Remaining: ddc, dart2js, dart2wasm, analysis_server, app-jit variants + deps generator (Step 3). See `rules_dart_scoping.md`. |
...
1. **`rules_dart` — the single biggest scope item.** DESIGN.md §4.3: this is the
   ...
   Steps 0–2 done + Step 4 in progress (sessions 11–12) — see `rules_dart_scoping.md`.**
   ...
   (the former byte-identical to GN); and three real tools (dtd, dds,
   frontend_server) are ported and run. Two more (dartdev, dart_runtime_service_vm)
   are blocked on out-of-band package_config/dependency staleness, not rule gaps.
   Remaining: deps generator (Step 3) + the other tools.
