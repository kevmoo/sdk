...
1. **`rules_dart` — the single biggest scope item.** DESIGN.md §4.3: this is the
   ...
   **Session 16 ported all 6 `compile_platform` web/wasm variants (additive
   ...
   vm_platform untouched).** Remaining clean work: `dartdevc`/`dart2js` app-jit/AOT
   (now unblocked) + `frontend_server`/`kernel-service` app-jit (need a staged
   platform dill) + deps generator (Step 3).
