...
1. **`rules_dart` — the single biggest scope item.** DESIGN.md §4.3: this is the
   ...
   Remaining clean work: `dartdevc`/`dart2js` (gated on the `compile_platform`
   **web variants** — see below) + `frontend_server`/`kernel-service` app-jit
   (need a staged platform dill) + deps generator (Step 3).
