...
1. **`rules_dart` — the single biggest scope item.** DESIGN.md §4.3: this is the
   ...
   **Session 16 ported all 6 `compile_platform` web/wasm variants (additive
   ...
   snapshots (consuming those web-variant dills; 0-line dump-diff vs GN).**
   Remaining clean work: `frontend_server`/`kernel-service` app-jit (need a staged
