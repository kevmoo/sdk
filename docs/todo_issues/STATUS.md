...
| M4 — multi-config `select()` | 🔴 ~10% (recon done) | still single-config `out/ReleaseX64`; no `select()`/overlay yet. **Release↔Debug axis scoped — `m4_multiconfig_scoping.md`: 10-token uniform flag delta (DEBUG/NDEBUG + `-fno-ident` + 2 `-Wno` + 4 linkopts), 0 target-set/dep change, `-O2` in both; gn-gen+desc sub-second & byte-deterministic across regens.** |
